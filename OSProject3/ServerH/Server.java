...
//Will be run on dc28 whose ip address is 10.176.69.59
public class Server {
    ...
    private final String[] transitions = { "ABCDEFGH", "EFGH", "H", "" };
    ...
    private int ru = 8;
    private String ds = "A";
    public final String fileString = "H wrote to file";
    ...
    public Server() {
        try {
            ...
            AsClient = new Socket(ipofE, 5050);
            System.out.println("Connected to E");
            ClientConnections.add(new ClientConnection("E", AsClient));
            AsServer = server.accept();
            System.out.println("E Connected");
            handler = new ServerConnection("E", AsServer);
            thread = new Thread(handler);
            thread.start();
    ...
    public void updateConnections() {
        ...
        // if (state > 0 && transitions[state - 1].length() <
